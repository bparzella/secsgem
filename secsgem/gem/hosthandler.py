...
class GemHostHandler(GemHandler):
    ...
    def send_remote_command(self, rcmd: int | str, params: list[str] | collections.OrderedDict[str, str]):
        ...
        remote_params = [
            {"CPNAME": param, "CPVAL": params[param]}
            if isinstance(params, collections.OrderedDict)
            else {"CPNAME": param[0], "CPVAL": param[1]}
            for param in params]

        s2f41 = self.stream_function(2, 41, {"RCMD": rcmd, "PARAMS": remote_params})

        # send remote command
        return self.settings.streams_functions.from_message(self.send_and_waitfor_response(s2f41))
    ...
    def get_process_program_list(self) -> secsgem.secs.data_item.StreamFunction:
        """Get process program list."""
        self._logger.info("Get process program list")

        # send remote command
        return self.settings.streams_functions.from_message(self.send_and_waitfor_response(self.stream_function(7, 19))).get()
    ...
    def go_online(self) -> str | None:
        ...
        # send remote command
        resp = self.settings.streams_functions.from_message(self.send_and_waitfor_response(self.stream_function(1, 17)))
        if resp is None:
            return None
    ...
    def go_offline(self) -> str | None:
        ...
        # send remote command
        return self.settings.streams_functions.from_message(self.send_and_waitfor_response(self.stream_function(1, 15))).get()
    ...
    def list_enabled_alarms(self):
        ...
        return self.settings.streams_functions.from_message(self.send_and_waitfor_response(self.stream_function(5, 7))).get()
    ...
    def _on_s05f01(
        ...
        """
        s5f1 = self.settings.streams_functions.from_message(message)
    ...
    def _on_s06f11(
        ...
        for report in function.RPT:
            report_dvs = self.report_subscriptions.get(report.RPTID.get(), None)
    ...
    def _on_s10f01(
        ...
        """
        s10f1 = self.settings.streams_functions.from_message(message)
