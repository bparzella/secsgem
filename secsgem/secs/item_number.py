...
class ItemNumber(Item):
    ...
    def validate_value(self, value: list[int | float] | int | float) -> typing.Any:
        ...
        if isinstance(value, self._type):  # isinstance already checked for int/float
            return [self._verify_value_in_bounds(value)]  # type: ignore[arg-type]
    ...
    def is_valid(cls, value: typing.Any, _length: int | None = None) -> bool:
        ...
        if isinstance(value, list):
            for item in value:
                if not isinstance(item, cls._type):  # isinstance already checked for int/float
                    return False

                if not cls._is_value_in_bounds(item):  # type: ignore[arg-type]
                    return False
            return True

        if isinstance(value, cls._type):
            return cls._is_value_in_bounds(value)  # type: ignore[arg-type] # isinstance already checked for int/float
...
class ItemF8(ItemNumber):
    ...
    _bytes = 8
    _struct_code = "d"
    _minimum_value = -1.79769e308
    _maximum_value = 1.79769e308
    _type = float
...
class ItemF4(ItemNumber):
    ...
    _bytes = 4
    _struct_code = "f"
    _minimum_value = -3.40282e38
    _maximum_value = 3.40282e38
    _type = float
